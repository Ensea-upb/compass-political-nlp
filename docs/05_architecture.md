...
- parent-child chunking in `document_pipeline.py` and `country_memory.py`: parent blocks preserve paragraph-level context, while child segments are citation units built from sentence/list fragments; very short fragments are merged and oversized fragments are split before indexing;
